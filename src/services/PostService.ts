...
class PostService {
    ...
    async create(post: PostDTO) {
        ...
        const author = await this.authorRepository.findOneBy({ name: post.author[0].name });
        ...
        if (author === null) {
            ...
            newMedia.title = post.media[0].title;
            // je recupère les données du post recu en entrée
            newMedia.category = post.media[0].category;
            // je recupère les données du post recu en entrée
            newMedia.theme = post.media[0].theme;
            // je recupère l'auteur id de l'auteur que je viens de créer
            newMedia.author_id = newAuthor[0].id!;
            ...
            if (isAuthorUpdated) {
                ...
                const media = await this.mediaRepository.findOneBy({ author_id: updatedAuthor!.id });
